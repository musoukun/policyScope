      ...
      <div className="flex h-screen bg-background">
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="border-b px-6 py-4">
            <h1 className="text-2xl font-bold">政党・政策Wiki チャット</h1>
          </div>
          ...
          <div className="max-w-4xl space-y-8 pb-24">
            {messagePairs.map((pair, index) => (
              <div key={pair.question.id} className="space-y-4">
                ...
                {/* ローディング表示 */}
                {!pair.answer && isLoading && index === messagePairs.length - 1 && (
                  <AnswerSkeleton />
                )}

                {index < messagePairs.length - 1 && <Separator className="my-6" />}
              </div>
            ))}
          </div>
        </ScrollArea>
            ...
            <div className="relative w-full">
              <div
                ...
                className={cn(
                  ...
                  isLoading && "ring-black/20 dark:ring-white/20"
                )}
